...
\item[2012-04-05] Version 2.2a: Added PDF with glyph tables (reproduction of Martin's PDF document in \TeX\ by Heiko Oberdiek).
 Renamed \path{ShortNinetyfive} into \path{ShortNinetyFive} in Type1 font (bug). Replaced \path{\EMail} by \path{\Email} and \path{\CheckedBox} by \path{\Checkedbox} due to name clashes with other \LaTeX\ packages.
